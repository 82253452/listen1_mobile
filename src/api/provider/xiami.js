...
async function getToken() {
  ...
  if (key && sig) {
    ...
    await MyStorage.setData(xiamiKey, JSON.stringify(token));
    return token;
  }
  return {};
}
...
async function showPlaylist(page) {
  const pageSize = 30;
  ...
  return await requestAPI(api, params).then((data) => {
    ...
    return {
      result: playlists,
      hasNextPage: page < data.result.data.pagingVO.pages,
    };
  });
}
